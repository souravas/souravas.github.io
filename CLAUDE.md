...
- **Start page**: a personal bookmarks dashboard at `/start`, with two designs laid out like the portfolio versions. [start/v2/](start/v2/) is the default (build-time copy to `/start`): pure black, v1's shared `public/fonts/`, and an inline prompt that filters the links in place. [start/v1/](start/v1/) is the original static rebuild of a self-hosted Homepage instance: dark slate, its own `fonts/`, a pre-blurred `serenity.webp` backdrop, and a modal quick launch. Bookmarks are plain `<li>` links and icons are `<symbol>`s in each page's inline SVG sprite — the two pages share the same markup, so edit both when changing links. Both are `noindex` and left out of the sitemap.
