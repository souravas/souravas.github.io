...
- **Start page**: [start/](start/) is a personal bookmarks dashboard at `/start` — a static rebuild of a self-hosted Homepage instance (bookmark groups, type-anywhere quick launch). Same self-contained layout as the versions, plus its own `fonts/` and a pre-blurred `serenity.webp` backdrop. Bookmarks are plain `<li>` links in `start/index.html`; icons are `<symbol>`s in the page's inline SVG sprite. It is `noindex` and left out of the sitemap.
