...
- **Start page**: [start/](start/) is a personal bookmarks dashboard at `/start` — a static rebuild of a self-hosted Homepage instance (clock, Open-Meteo weather, bookmark groups, type-anywhere quick launch). Same self-contained layout as the versions, plus its own `fonts/` and a pre-blurred `serenity.webp` backdrop. Bookmarks are plain `<li>` links in `start/index.html`; icons are `<symbol>`s in the page's inline SVG sprite. It is `noindex` and left out of the sitemap, and its CSP allows `connect-src https://api.open-meteo.com`.
- **Multi-page Vite build** with `v1/index.html`, `v2/index.html`, `v3/index.html`, and `start/index.html` as inputs, output to `dist/`; that directory is what gets uploaded as the Pages artifact. The `root-default` plugin in [vite.config.js](vite.config.js) copies the built `v1/index.html` to `dist/index.html`, so `/` serves v1 by default. Assets are hashed; each page's CSS bundle is inlined into its HTML by the `inline-css` plugin, so production has no render-blocking stylesheet request.
...
/start            → personal start page / bookmarks (noindex)
...
The dev server mirrors production routing: the `html-routes` plugin rewrites `/` → `/v1/index.html`, `/v2` → `/v2/index.html`, `/v3` → `/v3/index.html`, and `/start` → `/start/index.html`.
